...
async function loadActualTradingDays(date, count, loadDailyMarket) {
  ...
  for (let attempts = 0; days.length < count && attempts < 50; attempts += 1) {
    ...
    if (![0, 6].includes(cursor.getUTCDay())) {
      ...
      if (payload?.error) skipped.push({ date: candidate, reason: payload.error });
  ...
  for (const stock of payload.stocks || []) {
    ...
    for (const item of stock.classifications || []) {
      ...
      const tertiarySectors = [...new Set((stock.classifications || [])
        ...
        .filter(Boolean))];
      const key = `${primary}__${secondary}`; if (!themes.has(key)) themes.set(key, { key, primary, name: secondary, members: new Map() });
      themes.get(key).members.set(code, { code, name: stock.name || code, primary_affiliations: primaryAffiliations, tertiary_sectors: tertiarySectors });
    }
  }
  return [...themes.values()].map((item) => ({ ...item, members: [...item.members.values()] }));
}
...
function row(theme, daily, config) {
  ...
  return {
    name: theme.name, primary: theme.primary, key: theme.key, members: theme.members, dynamic_attribution: theme.dynamic_attribution, status: "pending_rank", score: null, rank: null,
...
export async function analyzeMainlines(date, loadDailyMarket, options = {}) {
  ...
  const themes = (await loadMembers()).map((theme) => row(theme, daily, config));
  ...
  const requestedTheme = options.requested_theme
    ...
    : null;
  return { version:config.version,date,data_scope:full?"full_market":"partial_market",mainline_eligible:full,market_summary:{valid_stock_count:daily.at(-1).totalStocks,history_dates:dates,uses_future_data:false},daily_core_module_eligibility:{minimum_valid_members:config.minimum_theme_members,active_signal_rule:"至少两只上涨，或至少一只大涨/涨停"},themes:scoreable,all_themes:themes,requested_theme:requestedTheme,mainline:{ranked:scoreable},ranking_definition:{daily_strength_rank:"仅按目标交易日的板块横截面表现排序",mainline_rank:"最近5日加权强度50%、资金持续认可度20%、抗分歧与修复能力20%、持续活跃能力10%；所有方向仅按该分数降序排列"},requested_history_start_date:historyStartDate || null,history_start_date:daily[0]?.date || null,history_end_date:daily.at(-1)?.date || null,history_trading_days:daily.length,warnings:[...(history.skipped.length ? [`已跳过 ${history.skipped.length} 个无行情日期`] : []),"评分仅使用目标日及此前行情；不再计算或使用生命周期状态。"],skipped_non_trading_dates:history.skipped };
}
